...
class SingleGPUModelBuilder(Generic[ModelType], ModelBuilderProtocol[ModelType], LoRAAdaptableProtocol):
    ...
    def _return_model(self, meta_model: ModelType, device: torch.device) -> ModelType:
        ...
        if uninitialized_params or uninitialized_buffers:
            ...
            # Materialize any remaining meta tensors on the target device
            # This handles cases where the checkpoint doesn't cover all model parameters
            def materialize_meta(module):
                for name, param in module.named_parameters(recurse=False):
                    if str(param.device) == "meta":
                        # Create new parameter on target device with same shape/dtype
            ...
            meta_model.apply(materialize_meta)
